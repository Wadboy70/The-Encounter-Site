  ...
  const recentContent = async () => {
    try {
      let live = await fetch(
        `https://www.googleapis.com/youtube/v3/search?key=${YOUTUBE_API_KEY}&channelId=UC_iDvnXDiDqg7u1cO-V41Mw&part=snippet,id&order=date&maxResults=10`
      )
        .then((val) => {
          return val.json();
        })
